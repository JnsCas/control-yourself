...
export class GmailClientDev extends GmailClientAbstract {
  ...
  async fetchEmailsIds(user: User): Promise<any[]> {
    logger.log(`Gmail Client Dev fetching emails ids for user ${user.id}`)
  ...
  async fetchMessage(user: User, messageId: string): Promise<any> {
    logger.log(`Gmail Client Dev fetching message ${messageId}`)
    ...
    return {
      ...
      payload: {
        ...
        headers: [
          {
            name: 'From',
            value: process.env.EMAIL_FROM || 'visa@notifications.visa.com',
          },
          {
            name: 'Subject',
            value: process.env.EMAIL_SUBJECT || 'Transaction Alert',
          },
