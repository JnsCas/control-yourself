...
async function showSummary(ctx: any, date: Date) {
  ...
  try {
    ...
    if (!expenses || expenses.length === 0) {
      ...
      await ctx.reply(
        `No expenses found for ${date.toLocaleString("default", { month: "long", year: "numeric" })}`,
      );
      return;
    }
    ...
    expenses.forEach((expense: any) => {
      ...
      if (suffix) {
        merchantDetails[prefix][suffix] =
          (merchantDetails[prefix][suffix] || 0) + amount;
      } else {
        // If no suffix, just add to the prefix total
        merchantDetails[prefix][""] =
          (merchantDetails[prefix][""] || 0) + amount;
      }
    });
    ...
    await ctx.reply(
      "Sorry, there was an error fetching your expenses. Please try again later.",
    );
  } finally {
    // Clear session state
